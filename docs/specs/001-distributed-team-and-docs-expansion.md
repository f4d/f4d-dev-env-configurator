...
we adopt are additive and fail-open, so this is not a safety gap; the requirement
...
- Adopt-out shape: its own skill (`/adopt-out`) or an action inside
  ...
  `/project-audit` action that reuses the doc-layout consumer-scan, recorded to
  `docs/log.md`. (A26)
